...
const SERVICE_ID_INTERNAL = "TO_Service_02_SQLite_Receiver_Clean";
...
function initDb() {
  try {
    ...
    log("INFO", `TO-2 SQLite database connected: ${DB_FILE}`);
    db.exec(
    ...
    log("INFO", `TO-2 BODI table ready.`);
    ...
    log("ERROR", `CRITICAL TO-2 DB INIT ERROR: ${err.message}`);
    console.error(err);
    process.exit(1);
  }
}
...
app.post("/receive-data", (req, res) => {
  ...
  const firstRecord = receivedData[0];
