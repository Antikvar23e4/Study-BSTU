...
const app = express();
const PORT = 3002;
const DB_FILE = path.join(__dirname, "to2.sqlite");
const SERVICE_ID_INTERNAL = "TO_Service_02_SQLite";
const MY_SUB_CODE = "T02REG";
...
function createAndInsertBodiRecord(objectIdNum) {
  ...
  const baseValue = STUDENT_NUMBER * 20;
  const rangeWidth = STUDENT_NUMBER * 8;
...
function generateDataContinuously() {
  const numObjectsToUpdate = Math.floor(Math.random() * 4) + 1;
  console.log(
    `[${MY_SUB_CODE}] Генерация ${numObjectsToUpdate} новых записей в БД...`
  );
  for (let k = 0; k < numObjectsToUpdate; k++) {
    const randomObjectIdNum = Math.floor(Math.random() * 5) + 1;
...
app.get("/data/since", (req, res) => {
  ...
  try {
    ...
    res.json(newData); // отправляем результат
  } catch (err) {
...
app.listen(PORT, () => {
  ...
  initDb();
  initialPopulation();
  setInterval(generateDataContinuously, GENERATION_INTERVAL_MS);
